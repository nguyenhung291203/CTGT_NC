...
public class Bai3 {
    ...
    public static int demSo(int[] arr, int target, int index) {
        if (index == arr.length) {
            return 0;
        }
        int dem = (target == arr[index]) ? 1 : 0;
        return dem + demSo(arr, target, index + 1);
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (String item : input) {
                ...
                int dem = demSo(arr, max, 0);
                System.out.printf("%s: %s%n", item, dem);
            }
